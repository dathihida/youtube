...
<head>
  ...
  <link rel="stylesheet" href="<c:url value='/templat/admin/plugins/tempusdominus-bootstrap-4/css/tempusdominus-bootstrap-4.min.css'/>">
  <!-- iCheck -->
                ...
                <a href="<c:url value='/admin/video?action=add'/>" class="nav-link active">
                  ...
                  <p>New or Edit</p>
						...
						<li class="nav-item menu-open"><a href="#"
							...
							<ul class="nav nav-treeview">
								<li class="nav-item"><a
									href="<c:url value='/admin/user?action=view'/>"
									class="nav-link active"> <i class="far fa-circle nav-icon"></i>
										<p>Overview</p>
								</a></li>
								<li class="nav-item"><a
									href="<c:url value='/admin/user?action=edit'/>"
            ...
            <div class="card-body">
              ...
              <div class="form-group">
               <label for="description">Email</label>
                <input type="text" id="email" value="${user!=null ? user.email : '' }" class="form-control">
              </div>
              <div class="form-group">
				<div class="row">
					<div class="col-9">
						<div class="row">
							<div class="col-12 form-group">
								<label for="poster">isAdmin</label>
								<!--<input type="radio" id="isAdmin" value="${video!=null ? video.poster : '' }" class="form-control">  -->
								<input ${user.isAdmin?'checked':''}class="form-check-input mb-5" style="padding-left: 5rem" name="isAdmin" type="radio"id="isAdmin" value="true">Admin
								<input ${user.isAdmin?'':'checked'}class="form-check-input" type="radio" name="isAdmin" id="isAdmin" value="false">User
...
<script>
	...
	$('#submitBtn').click(function() {
		...
		$.ajax({
			url:'/asm_java4/admin/user?action=edit&username=' + usernameOrigin,
			type: 'POST',
			data: formData
		}).then(function(data) {
			window.location.href = "http://localhost:8080/asm_java4/admin/user?action=view";
		}).fail(function(error) {
			$('#message').text('dda');
		});
	});
