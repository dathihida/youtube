...
<body>
	...
	<nav class="main-nav">
		<ul>
			...
			<li class="has-subnav"><a href="<c:url value='/trending'/>"> <span
					class="fa fa-2x"> <svg
							style="display: block; height: 24px; width: 24px; margin: auto;"
							viewBox="0 0 24 24">
							<path
	...
	<!-- sidebar -->
	<div class="container">
		<div class="row" style="padding-top: 2.6rem; padding-left: 10rem;">
			<c:forEach items="${videos}" var="video">
				<div class="col-lg-4 mb-1">
					<figure class="effect-ming tm-video-item">
						
						<a id="viewsUser" href="<c:url value='/video?action=watch&id=${video.href}'/>">
							<img src="${video.poster }" alt="Image" class="img-fluid"></a>
					</figure>
					...
					<div class="d-flex justify-content-between tm-text-gray">
						<span class="tm-text-gray-light">${video.shares }Shares</span> <span name="views" value="${video.views }">${video.views}Views</span>
					</div>
				</div>
				<input id="href" type="hidden" name="href" value="${video.href}">
				<input id="views" type="hidden" name="views" value="${video.views}">
			</c:forEach>
	...
	<div class="btn-toolbar" role="toolbar" style="padding-left: 42rem"
		aria-label="Toolbar with button groups">
		<div class="btn-group mr-2" role="group" aria-label="First group">
			<c:if test="${currentPage == 1 }">
					<a type="button" class="btn btn-secondary disabled">Previous</a>
			</c:if>
			<c:if test="${currentPage > 1 }">
			...
			<c:forEach varStatus="i" begin="1" end="${maxPage}">
					<a href="index?page=${i.index}"
						class="btn btn-secondary ${currentPage == i.index ? 'active' : ''}">${i.index}</a>
			</c:forEach>
			<c:if test="${currentPage == maxPage }">
				<a href="javascript:void(0);"
					class="btn btn-secondary disabled">NextPage</a>
			</c:if>

			<c:if test="${currentPage < maxPage }">
				<a href="index?page=${currentPage +1}"
					class="btn btn-secondary">NextPage</a>
			</c:if>
		</div>
	</div>
